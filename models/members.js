...
module.exports.retrieveAgeGroupSpending = function retrieveAgeGroupSpending(data, callback) {
    const sql = `SELECT * FROM get_age_group_spending($1, $2, $3)`;
    const VALUES = [data.gender, data.minTotalSpending, data.minMemberTotalSpending];
...
module.exports.retrieveAll = (callback) => {
    const SQLSTATEMENT = `SELECT * FROM member ORDER BY id`;
    return query(SQLSTATEMENT, (err, result) => {
        if (err) {
            return callback(err, null);
        }
        return callback(null, result);
